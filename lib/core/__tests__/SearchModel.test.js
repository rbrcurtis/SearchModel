...
class ModelWithHooks extends SearchModel_1.SearchModel {
    constructor() {
        super(...arguments);
...
describe('SearchModel', () => {
    ...
    describe('createIndex', () => {
        it('should create index with correct mapping', async () => {
            ...
            expect(SearchService_1.search.searchRequest).toHaveBeenCalledWith('PUT', '/test-index', expect.objectContaining({
                settings: expect.any(Object),
                mappings: expect.any(Object)
            }));
        });
        it('should handle existing index gracefully', async () => {
            const error = new SearchService_1.SearchError('resource_already_exists_exception');
            error.response = 'Index already exists: resource_already_exists_exception';
            mockedSearch.searchRequest.mockRejectedValue(error);
            await SearchModel_1.SearchModel.createIndex.call(TestModel);
            expect(SearchService_1.search.searchRequest).toHaveBeenCalled();
        });
        it('should throw error when indexName is not defined', async () => {
            class NoIndexModel extends SearchModel_1.SearchModel {
            }
            await expect(SearchModel_1.SearchModel.createIndex.call(NoIndexModel))
    ...
    describe('save', () => {
        it('should save new document', async () => {
            ...
            expect(SearchService_1.search.searchRequest).toHaveBeenCalledWith('PUT', `/test-index/_doc/${testId}?refresh=wait_for`, expect.objectContaining({
                id: testId,
                name: 'Test'
            }));
        ...
        it('should handle version conflicts', async () => {
            const testId = (0, id_1.id)();
            const versionError = new SearchService_1.VersionConflictError('Version conflict', 2, 1);
        ...
        it('should respect wait option', async () => {
            ...
            expect(SearchService_1.search.searchRequest).toHaveBeenCalledWith('PUT', `/test-index/_doc/${testId}`, expect.any(Object));
            mockedSearch.searchRequest.mockClear();
            await model.save();
            expect(SearchService_1.search.searchRequest).toHaveBeenCalledWith('PUT', `/test-index/_doc/${testId}?refresh=wait_for`, expect.any(Object));
        });
    });
    describe('delete', () => {
        it('should delete document', async () => {
            ...
            const model = new TestModel({ id: testId, name: 'Test' });
            await model.delete();
            expect(SearchService_1.search.searchRequest).toHaveBeenCalledWith('DELETE', `/test-index/_doc/${testId}`);
        });
        ...
        it('should throw error when indexName is not defined', async () => {
            ...
            const model = new NoIndexModel({ id: (0, id_1.id)() });
    ...
    describe('static methods', () => {
        describe('create', () => {
            ...
            it('should pass options to save', async () => {
                ...
                expect(SearchService_1.search.searchRequest).toHaveBeenCalledWith('PUT', expect.stringMatching(/^\/test-index\/_doc\/[^?]+$/), expect.any(Object));
            });
        });
        describe('find', () => {
            it('should find documents', async () => {
                mockedSearch.query.mockResolvedValue({
                    hits: [
                        { id: (0, id_1.id)(), name: 'Result 1' },
                        { id: (0, id_1.id)(), name: 'Result 2' }
                    ],
                    total: 2
                });
                const results = await TestModel.find(['name:Test']);
                expect(SearchService_1.search.query).toHaveBeenCalledWith('test-index', ['name:Test'], {});
            ...
            it('should pass options to query', async () => {
                ...
                expect(SearchService_1.search.query).toHaveBeenCalledWith('test-index', ['test'], { limit: 10, sort: 'name:desc' });
            });
        });
        ...
        describe('findOne', () => {
            it('should find first matching document', async () => {
                const testDoc = { id: (0, id_1.id)(), name: 'First' };
        ...
        describe('getById', () => {
            it('should get document by ID', async () => {
                ...
                expect(SearchService_1.search.getById).toHaveBeenCalledWith(TestModel, testId);
                expect(result).toBeInstanceOf(TestModel);
            });
