...
describe('ObjectArrayType - nested within nested arrays', () => {
    class TestModelNestedArrays extends SearchModel_1.SearchModel {
    }
    ...
    class TestModelNestedOuter extends SearchModel_1.SearchModel {
    }
    ...
    class TestModelNestedInner extends SearchModel_1.SearchModel {
    }
    ...
    it('should handle objectArray within objectArray with default (object) types', () => {
        const mapping = SearchModel_1.SearchModel.generateMapping.call(TestModelNestedArrays);
    ...
    it('should use nested type on outer array when specified', () => {
        const mapping = SearchModel_1.SearchModel.generateMapping.call(TestModelNestedOuter);
        const itemsMapping = mapping.mappings.properties.items;
        expect(itemsMapping.type).toBe('nested');
        expect(itemsMapping.properties.children.type).toBe('object');
    });
    it('should use nested type on inner array when specified', () => {
        const mapping = SearchModel_1.SearchModel.generateMapping.call(TestModelNestedInner);
        const itemsMapping = mapping.mappings.properties.items;
        expect(itemsMapping.type).toBe('object');
