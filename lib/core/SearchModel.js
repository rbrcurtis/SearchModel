...
class SearchModel {
    ...
    static async createIndex() {
        ...
        try {
            ...
            if (error instanceof SearchService_1.SearchError &&
                ...
                (0, logging_1.log)(`Index '${indexName}' already exists, skipping creation`, {
                    indexName,
                });
            }
            else {
    ...
    async save() {
        ...
        (0, logging_1.log)(`[SearchModel.save] Starting save for ${this.constructor.name} (ID: ${this.id})`, {
        ...
        if (this._isNewDocument) {
            this.createdAt = now;
            this.version = 1;
            (0, logging_1.log)(`[SearchModel.save] New document - setting version to 1`);
        }
        else {
            this.version += 1;
            (0, logging_1.log)(`[SearchModel.save] Existing document - incrementing version from ${currentVersion} to ${this.version}`);
        }
        this.updatedAt = now;
        const document = this.toDocument();
        (0, logging_1.log)(`[SearchModel.save] About to send request to Elasticsearch`, {
            indexName,
            documentId: this.id,
        ...
        try {
            const result = await SearchService_1.search.searchRequest('PUT', `/${indexName}/_doc/${this.id}?refresh=wait_for`, document);
            if (result && result._version) {
                this.version = result._version;
                (0, logging_1.log)(`[SearchModel.save] Updated version from ES response: ${result._version}`);
            }
            this._isNewDocument = false;
            this.clearChangedFields();
            await this.afterSave(saveEvent);
            (0, logging_1.log)(`[SearchModel.save] Save successful for ${this.constructor.name} (ID: ${this.id})`);
            return this;
        }
        catch (error) {
